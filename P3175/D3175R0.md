...
a bit nicer than `work | complete_on(over_there)`, and taken in isolation
...
1. If `on` is renamed `start_on`, and if a new algorithm, `on`, is introduced
    that completes where it started, do we also want to rename `transfer` to
    `complete_on`?

2. Do we want to add the `on(sndr, sched, continuation)` algorithm overload
    to permit scoped execution of continuations?
...
<blockquote><pre style="white-space: pre-wrap; font-size: 85%;">
  namespace <em>sender-adaptors</em> { <em>// exposition only</em>
    ...
    struct <del>transfer_t</del><ins>complete_on_t</ins>;
    <ins>struct on_t;</ins>
    struct schedule_from_t;
...
  }
  using <em>sender-adaptors</em>::<ins>start_</ins>on_t;
  using <em>sender-adaptors</em>::<del>transfer_t</del><ins>complete_on_t</ins>;
...
...
  ...
  inline constexpr <del>transfer_t transfer</del><ins>complete_on_t complete_on</ins>{};
...
8. *`write-env`* is an exposition-only sender adaptor such that, given
    subexpressions `sndr` and `env`, the expression
    <code><em>write-env</em>(sndr, env)</code> is equivalent to
    <code><em>write-sender</em>{{}, env, sndr}</code>, where *`write-sender`*
    is the following class template:
    ...
    struct <em>write-sender</em> {
      ...
      friend auto tag_invoke(get_completion_signatures_t, Self&& self, const auto& env) noexcept {
        using Env = decltype(<em>JOIN-ENV</em>(self.env, env));
        return completion_signatures_of_t&lt;copy_cvref_t&lt;Self, Sndr>, Env>{};
      }
...
"`on_t`" with "`start_on_t`". Additionally, change paragraph (4) as
follows: <span class="ednote">This is a drive-by bug fix.</spam>

<blockquote>

4. Let `out_sndr` and `env` be subexpressions such that `OutSndr` is
    `decltype((out_sndr))`. If <code><i>sender-for</i>&lt;OutSndr,
    start_on_t></code> is `false`, then the expressions
    `start_on.transform_env(out_sndr, env)` and
    `start_on.transform_sender(out_sndr, env)` are ill-formed; otherwise:

    - `start_on.transform_env(out_sndr, env)` is equivalent to:
    ...
    - `start_on.transform_sender(out_sndr, env)` is equivalent to:

        <pre style="white-space: pre-wrap; font-size: 85%;">
        auto&& [ign, sch, sndr] = out_sndr;
        return let_value(
          schedule(sch),
          [<ins>sch,</ins> sndr = std::forward_like&lt;OutSndr>(sndr)]() mutable {
            return <ins><em>write-env</em>(</ins>std::move(sndr)<ins>, <em>SCHED-ENV</em>(sch))</ins>; <ins>see [exec.adapt.general]</ins>
          });
        </pre>
</blockquote>
...
"`execution::complete_on` [exec.complete.on]", and within that subsection,
replace every instance of "`transfer`" with "`complete_on`" and every instance of
"`transfer_t`" with "`complete_on_t`".
...
1. The `on` sender adaptor has two forms:
    ...
    - one that starts a sender on an execution agent belonging to a particular
    ...
    - one that, upon completion of a sender, transfers execution to an execution
      ...
      and that then transfers execution back to the execution resource the sender
      completed on.
...
4. Let `out_sndr` and `env` be subexpressions such that `OutSndr` is
    ...
    - `on.transform_env(out_sndr, env)` is equivalent to:
        ...
        <pre style="white-space: pre-wrap; font-size: 85%;">
        auto&& [ign1, sch, ign2] = out_sndr;
        return <em>JOIN-ENV</em>(<em>SCHED-ENV</em>(sch), <em>FWD-ENV</em>(env));
        </pre>
    ...
    - `on.transform_sender(out_sndr, env)` is equivalent to:
        ...
        return let_value(
          schedule(sch),
          [sch, sndr = std::forward_like&lt;OutSndr>(sndr)]() mutable {
            return <em>write-env</em>(std::move(sndr), <em>SCHED-ENV</em>(sch)); // see [exec.adapt.general]
          });
        </pre>



<span class="ednote">TODO BUGBUG fix the above spec for `on.transform_sender` and spec the other form of `on`.</span>
