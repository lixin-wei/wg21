...
`ensure_started`, `start_detached`, and `split`? Those algorithms connect the
input sender with a receiver whose environment lacks a value for the
`get_scheduler` query. As specified in this paper, those algorithms will reject
`on` senders, which is bad from a usability point of view.
