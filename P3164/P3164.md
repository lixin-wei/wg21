...
Instead of "no overload found", users will get a diagnostic like:
"`static_assert: sender_to<Sndr, Rcvr> evaluated to false`", followed by a
(probably truncated) concepts backtrace. Buried in there somewhere _may_ be the
cause of the error for those entripid enough to dig for it.
...
instead of making the type ill-formed, it is better to name a type that
communicates the error to the user.

Algorithms like `sync_wait` can then `static_assert` that the result of
`completion_signatures_of_t` is a specialization of the `completion_signatures`
class template. If it instead names a type that is descriptive of the error, the
name of that type will appear prominently in the compiler's (blissfully short)
diagnostic.
