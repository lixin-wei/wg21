...
39. Let `Sndr` be a (possibly `const`-qualified) specialization
   ...
   the type `basic-operation<Sndr, Rcvr>` is well-formed, let `op` be an lvalue
   ...
   `op.start()`. Otherwise, the type denoted by `completion-signatures-for<Sndr,
   Env>`, if any, is not a specialization of `completion_signatures`. If
   `completion-signatures-for<Sndr, Env>` is well-formed and its type is not
   dependent upon the type `Env`, `completion-signatures-for<Sndr>` is
   well-formed and denotes the same type; otherwise,
   `completion-signatures-for<Sndr>` is ill-formed.
   \
   \
...
&nbsp;
  ...
  template&lt;class Sndr, class SetValue, class... Env>
    concept <em>sender-in-of-impl</em> =
      sender_in&lt;Sndr, Env...> &&
...
1. `get_completion_signatures` is a customization point object. Let `sndr` be an
   ...
   `CS` is:
	...
	2. Otherwise, `remove_cvref_t<NewSndr>::completion_signatures` if that type is
	   well-formed,

	3. Otherwise, if <code><em>is-awaitable</em>&lt;NewSndr,
   ...
   4. <ins>Otherwise, if `sizeof...(env) == 1` is `true`, then
...
<span class="ednote">To [exec.adapt.general], add paragraphs (5) and (6) as
follows:</span>

<blockquote><div class="ins">

5. Unless otherwise specified, an adaptor whose child senders are all
...
6. *Recommended practice:* Implementors are encouraged to use the completion
...
3. The name `split` denotes a pipeable sender adaptor object. For a
    ...
    `split(sndr)` mandates that</ins> <code>sender_in&lt;Sndr,
...
10. Otherwise, if any type `E` from set of types in `Vs` and `Es` fails to
