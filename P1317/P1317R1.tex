...
\item Let \texttt{\textit{ELEMS-OF}(T)} be the parameter pack
      \texttt{std::get<\textit{N}>(declval<T>())}, where
      \texttt{\textit{N}} is the pack of \texttt{size\_t}
      template arguments of the specialization of
      \texttt{index\_sequence} denoted by
      \texttt{make\_index\_sequence<tuple\_size\_t<T>\ >}.
\end{color}
\end{enumerate}
