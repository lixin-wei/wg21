...
#. In the algorithm `let_value(sndr, fun)`, if the input sender `sndr` has a
   completion scheduler for `set_value`, then the receiver connected to the
   sender returned from `fun` shall expose that scheduler as the current
   scheduler of the receiver's environment.

   In other words, if the input sender `sndr` completes with values `vs...`,
   then the result of `fun(vs...)` will be connected to a receiver `r` such that
   `get_scheduler(get_env(r))` is equal to
   `get_completion_scheduler<set_value_t>(get_env(sndr))`.

   The same is true also of the domain query: `get_domain(get_env(r))` is equal
   to the domain of the input sender as computed by the steps in (2) above.

   So for `let_value`, likewise also for `let_error`, using `set_error_t` when
   querying for the input sender's completion scheduler. (`let_stopped` needs no
   modification because the nullary function passed to `let_stopped` can only
   have a single return type; hence there is only one secondary sender type and
