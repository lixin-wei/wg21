...
1. This section makes use of the following exposition-only entities.
    ...
    1.  ```
        ...
        _Effects:_ Let `@_COMPL-DOMAIN_@(T, S)` be the type of the expression
            `get_domain(get_completion_scheduler<T>(get_env(S)))`. If
            ...
            `@_completion-domain_@<Default>(sndr)` is a default-constructed prvalue of
            that type. If all of those types are ill-formed, `@_completion-domain_@<Default>(sndr)`
            is a default-constructed prvalue of type `Default`.
    ...
    3.  ```
        ...
        _Effects:_ Equivalent to:
          ...
          --- If `@_sender-for_@<Sender, transfer_t>` is `true`, then
              ...
              is the scheduler that was used to contruct `sndr`,
...
5. [For a sender `s2` returned from `when_all` or `when_all_with_variant`,
    ...
    `@_get-domain-early_@(s@<sub>_0_</sub>@)` when `Q` is `get_domain`; otherwise,
...
2. The name `start_detached` denotes a customization point object. For some
    ...
    > :::add
    > ```
    > apply_sender(@_get-domain-early_@(s), start_detached, s)
    > ```
    > :::
...
> > :::add
> > ```
> > apply_sender(@_get-domain-early_@(s), sync_wait, s)
> > ```
> > :::
...
> > :::add
> > ```
> > apply_sender(@_get-domain-early_@(s), sync_wait_with_variant, s)
> > ```
> > :::
