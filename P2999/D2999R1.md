...
1. This section makes use of the following exposition-only entities.
    ...
    5.  ```
        ...
        _Returns:_ A prvalue of type `@_basic-sender_@<Tag, decay_t<Data>, decay_t<Child>...>`
            ...
            _`data`_ and `@_child<sub>__n_</sub>@...` members have
...
1. `transfer` adapts a sender into a sender with a different associated
   `set_value` completion scheduler. [_Note_: it results in a transition
...
1. `schedule_from` schedules work dependent on the completion of a sender onto a
    scheduler's associated execution resource. [_Note_: `schedule_from` is
...
2. The name `start_detached` denotes a customization point object. For some
    ...
    `sender`, `start_detached` is ill-formed. Otherwise, the expression
    `start_detached(s)` is expression-equivalent to:

    1. `tag_invoke(start_detached, @[get_completion_scheduler<set_value_t>(get_env(s))]{.rm}[_get-sender-domain_(s)]{.add}@, s)`, if that expression is valid.
    ...
    3. Otherwise:
        ...
        2. Calls `connect(s, r)`, resulting in an operation state
            `op_state`, then calls `start(op_state)`.
