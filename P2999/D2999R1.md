...
2. The names `let_value`, `let_error`, and `let_stopped` denote customization
    ...
    3. <div class="rm" style="color: #bf0303">Otherwise, given a receiver
        ...
        >         expression-equivalent to
        >         `@[_completion-function_(std::move(out_r'), args\...)]{.rm}[_CF_(out_r2, args\...)]{.add}@`,
...
3. Given subexpressions `s2` and `e`, where `s2` is a sender returned
    ...
    3. For each `Sig2@<sub>_i_</sub>@` in `Sigs2...`, let
        ...
        `sender_in<S3@<sub>_i_</sub>@, @[E]{.rm}[E2]{.add}@>` is not satisfied,
        [where `E2` is the type of `@_INNER-ENV_@(get_env(s), e)`,]{.add} then the
...
4. If [a sender `S` returned from]{.add} `@_let-cpo_@(s, f)` [is connected to a
