...
Add a paragraph to __§11.9 [exec.snd]__
...
1. This section makes use of the following exposition-only entities.
    ...
    5.  ```txt
        ...
        _Returns:_ A prvalue of type `@_basic-sender_@<Tag, decay_t<Data>, decay_t<Child>...>`
            ...
            _`data`_ and <code><i>child</i><sub>n</sub>\...</code> members have
...
To __§11.9.5.2 [exec.just]__, add a paragraph 6 as follows:
...
2. The name `transfer_just` denotes a customization point object. For some
    subexpression `s` and pack of subexpressions `vs`, let `S` be
    `decltype((s))` and let `Vs` be the template parameter pack
    `decltype((vs))...`. If `S` does not satisfy `scheduler`, or any type `V` in
    `Vs` does not satisfy <code><i>movable-value</i></code>,
    `transfer_just(s, vs...)` is ill-formed. Otherwise, `transfer_just(s, vs...)`
    is expression-equivalent to:

      > <div class="add" style="color: #006e28">
      > ```txt
      > transform_sender(
      >   @_query-or-default_@(get_domain, s, default_domain()),
      >   @_make-sender_@(transfer_just, @_tuple-like_@{s, vs...}));
      > ```
      > </div>


  > 1. [`tag_invoke(transfer_just, s, vs...)`, if that expression is valid.]{.rm}
...
3. Let `as` be a pack of rvalue subexpressions of types `decay_t<Vs>...`
    ...
    selected by `tag_invoke`]{.rm}[a sender `S` returned from `transfer_just(s, vs...)`
    ...
    datums `as`, the behavior of calling [`transfer_just(s, vs...)`]{.rm}
    ...
    * _Mandates:_ `sender_of<@[R]{.rm}[S]{.add}@, set_value_t(decay_t<Vs>...), E>`,
        ...
        [`transfer_just(s, vs...)`]{.add}, and `E` is the type of an environment.
...
:::add
4. For some subexpression `s` and pack of subexpressions `vs`, let `S` be a
    subexpression refering to an object returned from `transform_just(s, vs...)`
    or a copy of such. Then
    `get_completion_scheduler<set_value_t>(get_env(S)) == s` is `true`,
    `get_completion_scheduler<set_stopped_t>(get_env(S)) == s` is `true`, and
    `get_domain(get_env(S))` is expression-equivalent to `get_domain(s)`.
...
5. Let `s` and `e` be subexpressions such that `S` is `decltype((s))`. If
  ...
  > [This causes the `transform_just(s, vs...)` sender to become
  > `transform(just(vs...), s)` when it is connected with a receiver whose
  > domain does not customize `transform_just`.]{.note}

:::

Update __§11.9.6.3 [exec.on]__ as follows:
...
3. The name `on` denotes a customization point object. For some subexpressions
    ...
    `Sch` does not satisfy `scheduler`, or `S` does not satisfy `sender`, `on`
    is ill-formed. Otherwise, the expression `on(sch, s)` is
...
Update __§11.9.6.4 [exec.transfer]__ as follows:
...
2. The name `transfer` denotes a customization point object. For some
    ...
    satisfy `sender`, `transfer` is ill-formed. Otherwise, the expression
    `transfer(s, sch)` is expression-equivalent to:
    ...
    1. `tag_invoke(transfer, get_completion_scheduler<set_value_t>(get_env(s)), s, sch)`,
        ...
        * <i>Mandates:</i> The type of the `tag_invoke` expression above satisfies `sender`.
