...
> arbitrary type and then pass it to `transform_sender` with the currently
> active domain (likely computed by asking the child sender(s) or the receiver's
> environment depending on whether this is early or late customization).
...
#. Add a new, non-customizable
   `apply_sender(@_domain_@, @_tag_@, @_sender_@, @_args_@...) -> @_result_@` API. It
...
The fo`llowing proposed changes are relative to [@P2300R7].
