...
2. The name `transfer_just` denotes a customization point object. For some
    ...
    is expression-equivalent to[:]{.rm}
    [`transform_sender(@_query-or-default_@(get_domain, s, default_domain()),
    @_basic-sender_@{transfer_just, @_tuple-like_@{s, vs...}})`]{.add}.

    <div class="rm" style="color: #bf0303">
    1. `tag_invoke(transfer_just, s, vs...)`, if that expression is valid. Let
        `as` be a pack of rvalue subexpressions of types `decay_t<Vs>...`
        ...
        * _Mandates:_ `sender_of<R, set_value_t(decay_t<Vs>...), E>`,
            where `R` is the type of the `tag_invoke` expression above, and
            `E` is the type of an environment.

    2. Otherwise, `transfer(just(vs...), s)`.
    </div>

    <div class="add" style="color: #006e28">
    > 1. For expressions `sndr` and `env`, the expression
    >    `transfer_just_t().transform_sender(sndr, env)` is equivalent to:
    >
    > > ```txt
    ...
    > >          domain,
    > >          transfer(execution::transform_sender(domain, just(std::move(vs)...), env), std::move(s)),
    > >          env);
    > > ```
