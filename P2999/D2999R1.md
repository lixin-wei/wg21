...
4. For some subexpression `s` and pack of subexpressions `vs`, let `S` be a
    ...
    Then `get_completion_scheduler<set_value_t>(get_env(S)) == s` is `true`
    and `get_completion_scheduler<set_stopped_t>(get_env(S)) == s` is `true`.

5. For subexpressions `s` and `e`, the expression
    `transfer_just_t().transform_sender(s, e)` is equivalent to:
  ...
  > [Thus causing the `transform_just(s, vs...)` sender to become `transform(just(vs...), s)`
  > when it is connected with a receiver whose domain does not customize `transform_just`.]{.note}
:::
