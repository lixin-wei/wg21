...
an abstract tag type is a short hop. We call this abstract tag type a _domain_.
Several different scheduler types may all want to use the same set of algorithm
implementations; those schedulers can all use the same domain type for the
purpose of dispatching.

If `when_all`'s child senders all share a domain, we know the domain of the
...
<table style="caption-side: bottom;">
  <caption>Table 1: The differences between `transform_sender` and `apply_sender`</caption>
  <thead>
    <tr>
...
and then pass it to `transform_sender` with a domain to place it in its final
form.
...
#. In the algorithm `let_value(snd, fun)`, all of the sender types that the
    ...
    require that all the result senders share a domain with the predecessor
    ...
    the scheduler `get_scheduler(get_env(R))` has the same domain
    as that of `schA`.
...
#. Add a new
   ...
   - called the sender's domain, the algorithm's tag, the sender, and any
...
2. The names `let_value`, `let_error`, and `let_stopped` denote customization
    point objects. Let the expression _`let-cpo`_ be one of `let_value`,
    `let_error`, or `let_stopped` [and let _`set-cpo`_ be the completion
    function that corresponds to _`let-cpo`_ (`set_value` for `let_value`,
    etc.)]{.add}. For subexpressions `s` and `f`, let `S` be
    `decltype((s))`, let `F` be the decayed type of `f`, and let `f'` be an
    ...
    3. <div class="rm" style="color: #bf0303">Otherwise, given a receiver
        ...
        >         `@_INNER-ENV_@(get_env(s), get_env(out_r2))`, where
        >         `@_INNER-ENV_@(SE, RE)` is an environment `E` such that:
        >
        ...
        >
        >         --- For all other query objects _`Q`_ and
        >               arguments `args...`, `@_Q_@(E, args...)` is
        >               expression-equivalent to `@_Q_@(RE, args...)`;
        >         </div>
        >
...
3. Given subexpressions `s2` and `e`, where `s2` is a sender returned
...
<!-- -->
5. If [a sender `S` returned from]{.add} `@_let-cpo_@(s, f)` [is connected to a
...
5. Let `s` and `e` be subexpressions such that `S` is `decltype((s))`. If
    ...
    domain does not customize `when_all_with_variant`.]{.note}
:::
...
3. Let `s` and `e` be subexpressions such that `S` is `decltype((s))`. If
    ...
    domain does not customize `transfer_when_all`.]{.note}
:::
...
4. Let `s` and `e` be subexpressions such that `S` is `decltype((s))`. If
    ...
    with a receiver whose domain does not customize
