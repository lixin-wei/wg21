...
_`basic-sender`_ as follows:]{.ednote}

> 24. _Returns_: A prvalue of type ```@_`basic-sender`_@<Tag, decay_t<Data>,
>     decay_t<Child>...>``` that has been direct-list-initialized with the
>     forwarded arguments, where _`basic-sender`_ is the following exposition-only
...
>
> 42. _Throws_: An exception of an unspecified type if the expression
>     ```@_`impls-for`_@<Tag>::template @_`check-types`_@<Sndr, Env...>()``` is
>     ill-formed.
>
> 43. _Effects_: Equivalent to
>
>     ```c++
>     @_`impls-for`_@<Tag>::template @_`check-types`_@<Sndr, Env...>();
>     return CS();
...
>
>     - [3.3]{.pnum} Otherwise, ```(throw @_`dependent-sender-error`_@(), completion_signatures())```
>       if ```@_`dependent-sender-without-env`_@<Sndr, Env...>``` is `true`.
>
>     - [3.4]{.pnum} Otherwise, ```(throw @_`unspecified`_@, completion_signatures())``` if
...
>     ```
>
>     [where the member function get_completion_signatures returns an object of a
>     type that is not a specialization of the completion_signatures class
>     template.]{.rm}
