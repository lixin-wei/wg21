...
> ```@_`MATCHING-SIG`_@(Sig, F)``` is `true` ([exec.general]). Together, a sender type and
...
> sender type whose completion signatures are knowable independent of an execution
> environment is known as a <dfn>non-dependent sender</dfn>.]{.add}
...
> 
> [2]{.pnum} For types `Sndr` and [pack]{.add} `Env`, [let `CS` be
...
> it is a specialization of the `completion_signatures` class template.]{.add}
>
> :::
...
[In [exec.snd.expos], insert the following paragraph after para 22 and before para 23
...
[In [exec.snd.expos] para 23 add the mandate shown below:]{.ednote}
...
[In [exec.snd.expos] para 24, change the definition of the exposition-only
...
<span class="ednote">Change [exec.awaitable] p 1-4 as follows:</span>
...
> 
> [2]{.pnum} For a subexpression `c`, let `GET-AWAITER(c, p)` be expression-equivalent to
...
>    
> [Let `GET-AWAITER(c)` be expression-equivalent to
...
> ```coroutine_handle<@_`none-such`_@>``` behaves as
> `coroutine_handle<void>`.]{.add}
> 
...
[Change [exec.adapt.general] as follows:]{.ednote}
...
> 
> - [3.?]{.pnum} [_Recommended practice_: Implementors are encouraged to use the
>   completion signatures of the adaptors to communicate type errors to users and to propagate
>   any such type errors from child senders.]{.add}
...
[Change [exec.then] para 4 as follows:]{.ednote}
...
[Change [exec.let] paras 5 and 6 and insert a new para after 6 as follows:]{.ednote}
...
[Change [exec.bulk] para 3 and insert a new para after 5 as follows:]{.ednote}
...
[Change [exec.split] para 3 and insert a new para after 3 as follows:]{.ednote}
...
[Change [exec.stopped.opt] as follows. Note: this includes the proposed resolution to
...
[Change [exec.run.loop.types] para 5 as follows:]{.ednote}
