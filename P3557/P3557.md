...
[Change [execution.syn] as follows (changes relative to what is already
suggested above):]{.ednote}

> ```c++
> @<span style="color:blue;text-style:italic !important;">&hellip;&nbsp;as&nbsp;before&nbsp;&hellip;</span>@
> 
> namespace std::execution {
>   @<span style="color:blue;text-style:italic">&hellip; as before &hellip;</span>@
> 
...
>     concept @_`valid-completion-signatures`_@ = @_`see below`_@;            // exposition only
> 
>   struct @_`dependent-sender-error`_@ : exception {};                   // exposition only
> 
>   // [exec.getcomplsigs]
>   template<class Sndr, class... Env>
>     consteval auto get_completion_signatures() -> @_`valid-completion-signatures`_@ auto;
...
> 
>   @[_`template<bool PotentiallyThrowing>`_]{.add}@
>     @[_`inline constexpr auto eptr_completion_if = completion_signatures();`_]{.add}@
>   
>   @[_`template<>`_]{.add}@
>     @[_`inline constexpr auto eptr_completion_if<true> =`_]{.add}@
>       @[_`completion_signatures<set_error_t(exception_ptr)>();`_]{.add}@
>   
>   @[_`// [exec.util.cmplsig.getchild]`_]{.add}@
>   @[`template <class Parent, class Child, class... Env>`]{.add}@
>     @@[```consteval auto get_child_completion_signatures() -> @_`valid-completion-signatures`_@ auto;```]{.add}@@
>
>   @[_`// [exec.util.cmplsig.make]`_]{.add}@
>   @@[```template <@_`completion-signature`_@... ExplicitSigs, @_`completion-signature`_@... DeducedSigs>```]{.add}@@
>     @[`constexpr auto make_completion_signatures(DeducedSigs*... sigs) noexcept`]{.add}@
>       @@[```-> @_`valid-completion-signatures`_@ auto;```]{.add}@@
>
>   @[_`// [exec.util.cmplsig.invalid]`_]{.add}@
>   @[`template <class... Types, class... Values>`]{.add}@
>     @[`[[noreturn, nodiscard]]`]{.add}@
>     @[`consteval auto invalid_completion_signature(Values... values)`]{.add}@
>       @[`-> completion_signatures<>;`]{.add}@
>
>   @[_`// [exec.util.cmplsig.trans]`_]{.add}@
>   @@[```template<```]{.add}@@
>       @@[```@_`valid-completion-signatures`_@ Completions,```]{.add}@@
>       @@[```class ValueTransform = @_`see below`_@,```]{.add}@@
>       @@[```class ErrorTransform = @_`see below`_@,```]{.add}@@
>       @@[```class StoppedTransform = @_`see below`_@,```]{.add}@@
>       @@[```@_`valid-completion-signatures`_@ ExtraCompletions = @_`see below`_@>```]{.add}@@
>     @@[```constexpr auto transform_completion_signatures(```]{.add}@@
>         @@[```Completions completions,```]{.add}@@
>         @@[```ValueTransform value_transform = {},```]{.add}@@
>         @@[```ErrorTransform error_transform = {},```]{.add}@@
>         @@[```StoppedTransform stopped_transform = {},```]{.add}@@
>         @@[```ExtraCompletions extra_completions = {});```]{.add}@@
> 
>   // [exec.run.loop], run_loop
>   class run_loop;
> }
...
> ::: add
>
> **[33.7.?]{.pnum} Completion tags [exec.set.tag]**
>
...
> with instances comparing equal to each other. Each also models
> `equality_comparable_with` the other two, with objects of different types
> comparing not equal. For two completion tag objects `a` and `b`, `a == b` and
> `a != b` are core constant expressions and are not potentially throwing.
> [`set_value_t() == set_value_t()` is a constant expression that evaluates to
> `true`, and `set_value_t() == set_error_t()` is a constant expression that
> evaluates to `false`.]{.example}
>
> :::


[Give the `completion_signatures` class template some `constexpr` members for
manipulating its instances. Instead of the change suggested above to
[exec.util.cmplsig] para 8, split para 8 into two after `struct
completion_signatures` and modify as follows:]{.ednote}
...
[After [exec.util.cmplsig], insert a three new sections as follows:]{.ednote}

::: add

> **[33.10.?]{.pnum} `execution::get_child_completion_signatures` [exec.util.cmplsig.getchild]**
>
> ```c++
> template <class Parent, class Child, class... Env>
>   consteval auto get_child_completion_signatures() -> @@```@_`valid-completion-signatures`_@ auto```@@;
> ```
>
> [?]{.pnum} _Effects_: Equivalent to:
>
> > ```cpp
> > using CvChild = decltype(std::forward_like<Parent>(declval<Child&>()));
> > return get_completion_signatures<CvChild, @_`FWD-ENV-T`_@(Env)...>();
> > ```
>
