...
6. Remove the `transform_completion_signatures` alias template.
...
> be a pack expressions. If `sizeof...(env) == 0` is `true`, let `new_sndr` be
> `sndr`; otherwise, let `new_sndr` be the expression
> ```transform_sender(decltype(@_`get-domain-late`_@(sndr, env...)){}, sndr, env...)```,
> and let `NewSndr` be `decltype((new_sndr))`. If `sizeof...(env) > 1` is `true`
> then `get_completion_signatures(sndr, env...)` is ill-formed;
> otherwise, it is expression-equivalent to `(void(sndr), void(env)..., CS())`
> except that `void(sndr)` and `void(env)...` are indeterminately sequenced,
> where `CS` is:
>
