...
TODO: type-check that result datums that need to be copied into the operation
states are actually decay-copyable.

TODO: Mandate that `transform_sender` returns a sender that either type-checks
successfully, or else is a dependent sender.

TODO: doesn't get_completion_signatures need to do something with awaitables?
...
* Add support for awaitables back to `get_completion_signatures`.
...
* Integrate proposed fix for `stopped_as_optional` bug [cplusplus/sender-receiver#311](https://github.com/cplusplus/sender-receiver/issues/311).
...
>     return Completions();
>   else
>     // invalid_completion_signature throws unconditionally. It's return type
...
change. It can be found on [Compiler
Explorer](https://godbolt.org/z/P8YdPhba1)[^1]. This implementation includes
all the design elements that would stress the completion signature computation
including:
...
[^1]: [https://godbolt.org/z/P8YdPhba1](https://godbolt.org/z/P8YdPhba1)
...
> ```
>
> [?]{.pnum} Let `FwdEnv` be a pack of the types ```decltype(@_`FWD-ENV`_@(declval<Env>()))...```,
...
>
> > ```c++
> > (get_completion_signatures<@_`child-type`_@<Sndr, Is>, FwdEnv...>(), ...)
> > ```
>
> [?]{.pnum} _Remarks_: For any types `T`, `S`, and pack `E`, let `e` be the
> expression ```@_`impls-for`_@<T>::@_`check-types`_@<S, E...>()```. Then one of
...
>
> - [?.1]{.pnum} If _`check-types`_ is well-formed and a core constant
    expression, let `op` be an lvalue subexpression whose type is
...
>
>   _Mandates_: ```@[`sender<Sndr>`]{.rm}[`sender_in<Sndr, env_of_t<Rcvr>>`]{.add}@ && receiver<Rcvr>``` is `true`.
...
>
> [6]{.pnum} The member <span style="color:blue;font-style=italic">&hellip; as before &hellip;</span>
>
> ::: add
>
> ```c++
...
>
> > ```c++
> > get_completion_signatures<schedule_result_t<@_`data-type`_@<Sndr>>>,
...
>       throw @_`unspecified`_@;
>     else if constexpr (sizeof...(Env) == 1 && !sender_in<invoke_result_t<LetFn, Ts...>, Env...>)
>       throw @_`unspecified`_@;
>   };
>   cs.@_`for-each`_@(@_`overload-set`_@(fn, [](auto){}));
>   ```
...
>   template<class Sndr, class... Env>
>   static constexpr void @_`check-types`_@() {
>     auto cs = get_completion_signatures<@_`child-type`_@<Sndr>, decltype(@_`FWD-ENV`_@(declval<Env>()))...>();
>     if constexpr (!requires { typename @_`single-sender-value-type`_@<@_`child-type`_@<Sndr>, Env...>; })
>       throw @_unspecified_@;
>     else if constexpr (same_as<void, @_`single-sender-value-type`_@<@_`child-type`_@<Sndr>, Env...>>)
>       throw @_unspecified_@;
>   }
> };
> ```
...
> [otherwise, if ```sender_in<@_`child-type`_@<Sndr>, Env>``` is `false`,
...
> ```c++
> auto&& [_, _, child] = sndr;
> using V = @_`single-sender-value-type`_@<@@[```@_`child-type`_@<```]{.add}@@Sndr@[`>`]{.add}@, Env>;
> return let_stopped(
...
> ```completion_signatures<Fns...>::@_`count-of`_@(tag)``` is
> initialized with the count of function types in `Fns...` that are of the
> form `Tag(Ts...)` where `Ts` is a pack of types.]{.add}
