...
R1
----
* Fix the specification of `std::execution::get_completion_signatures`.
...
on [Compiler Explorer](https://godbolt.org/z/e9G95WofW)[^1].

[^1]: [https://godbolt.org/z/e9G95WofW](https://godbolt.org/z/e9G95WofW)
...
>
> - [?.2]{.pnum} Otherwise,
>   ```@_`CHECKED-COMPLSIGS`_@(@_`get-complsigs`_@<NewSndr>())``` if that
>   expression is well-formed.
>
> - [?.3]{.pnum} Otherwise,
>   ```@_`CHECKED-COMPLSIGS`_@(typename remove_reference_t<NewSndr>::completion_signatures())```
>   if that expression is well-formed.
>
> - [?.4]{.pnum} Otherwise, ```throw @_`dependent-sender-error`_@()``` if
>   `sizeof...(Env) == 0` is `true`,
>
> - [?.5]{.pnum} Otherwise, ```throw @_`unspecified`_@```.
>
> :::
>
> ::: rm
