...
All we need is one new sender attribute query, tentatively called `get_domain_late`. The
...
1. Add a non-forwarding `get_domain_late` query with no default implementation.

2. Tweak the definitions of _`SCHED-ATTRS`_ and _`SCHED-ENV`_ to avoid forwaring the
   `get_domain` query.

3. Simplify the definition of the exposition-only _`completion-domain`_ helper,
   which no longer needs a configurable default.

4. Specify that `get_domain_late(get_env(schedule_from(sch, sndr)))` returns the domain of
   `sch`.

5. Specify that `get_domain_late(get_env(continues_on(sndr, sch)))` returns the domain of
   `sndr` (if it has one).

6. The expression _`get-domain-late`_`(sndr, env, def)` should be equivalent to:

   1. `get_domain_late(get_env(sndr))` if that expression is well-formed.
...
7. Specify that `sync_wait` and `sync_wait_with_variant` use
...
> >   @@[```struct get_domain_late_t { @_`unspecified`_@ };```]{.add}@@
...
> >   @[`inline constexpr get_domain_late_t get_domain_late{};`]{.add}@
...
[After [exec.get.domain]{.sref} add a new subsection [exec.get.domain.late] as follows:]{.ednote}


::: add

> **[33.5.?] `execution​::​get_domain_late` [exec.get.domain.late]**
> 
> [1]{.pnum} `get_domain_late` asks a queryable object for the domain tag to use in `connect`
> and `get_completion_signatures` to find a sender transformation.
> 
> [2]{.pnum} The name `get_domain_late` denotes a query object. For a subexpression `env`,
> `get_domain_late(env)` is expression-equivalent to
> ```@_`MANDATE-NOTHROW`_@(@_`AS-CONST`_@(env).query(get_domain_late))```.
...
>    
>    - [6.3]{.pnum} `o1.query(get_domain)` is expression-equivalent to `sch.query(get_domain)`[.]{.rm}
>       [if that expression is well-formed, and `default_domain()` otherwise, and]{.add}
>    
...
>  
>        * ```get_domain@[`_late`]{.add}@(get_env(sndr))```
...
> > ```
>
> [where `E` is a queryable object such that `E.query(get_domain_late)` is expression-equivalent
