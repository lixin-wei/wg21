...
4. Specify that `get_domain_override(get_env(schedule_from(sch, sndr)))` returns the
   domain of `sch`.

5. Specify that `get_domain_override(get_env(continues_on(sndr, sch)))` returns the domain
