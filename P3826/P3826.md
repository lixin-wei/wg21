...
>    ```@_`sender-for`_@<Sndr, bulk_t>```{.cpp} is `false`, then the expression
>    ```bulk.@[`transform_sender`]{.rm}[_`transform-sender`_]{.add}@(sndr, env)```{.cpp}
...
>    > ```
> 
>    [_Note 1_: This causes the `bulk(sndr, policy, shape, f)` sender to be expressed in terms
>    of `bulk_chunked(sndr, policy, shape, f)` when it is connected to a receiver [whose
>    execution domain does not customize `bulk`]{.rm}. — _end note_]
