...
>
>    - [5.1]{.pnum} ```sch.@_`bulk-transform`_@(sndr, env)```{.cpp} if that expression is well-formed;
>      otherwise,
>
>    - [5.2]{.pnum} ```sch.@_`bulk-transform`_@(@_`as-bulk-chunked`_@(sndr), env)```{.cpp} if
>      that expression is well-formed; otherwise,
> 
>    - [5.3]{.pnum} ```@_`bulk-transform`_@(sch, sndr, env)```{.cpp} is ill-formed.
> 
...
>    expression-equivalent to:
>
>    - [6.1]{.pnum} ```@_`bulk-transform`_@(get_completion_scheduler<set_value_t>(get_env(sndr)), sndr, env)```{.cpp}
>      if that expression is well-formed, except that `sndr` is evaluated only once;
>      otherwise,
>
>    - [6.2]{.pnum} ```@_`bulk-transform`_@(get_scheduler(env), sndr, env)```{.cpp} if that
>      expression is well-formed, except that `env` is evaluated only once; otherwise,
>
>    - [6.3]{.pnum} ```@_`bulk-algo`_@.@_`transform-sender`_@(sndr, env)```{.cpp} is ill-formed.
>
> :::
...
namespace std::execution {
  class task_scheduler {
    class @_`ts-sender`_@;                    // exposition only
  ...
  public:
    ...
    template <class Sndr, class Env>
    @_`ts-bulk-sender`_@ @_`bulk-transform`_@(Sndr&& sndr, const Env& env);
