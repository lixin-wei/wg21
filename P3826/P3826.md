...
- `execution::task` and `execution::task_scheduler` (C++26 will _still_
  not have a standard coroutine task type).
...
This is the option this paper proposes. We ship everything currently in the Working
Draft but remove the ability to customize the algorithms. This gives us a free hand
to design a better customization mechanism for C++29 -- provided we have high
certainty that those new customization hooks will not break existing behavior.
...
that solution or one like it later without portability problems.
...
The downside of this approach is that we will still have to support it even when a
more general algorithm customization mechanism is available. That doesn't seem
terribly onerous to me, but that is for SG1/LEWG to decide.
