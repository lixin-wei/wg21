...
12. Fix a bug in the `on(sndr, sch, closure)` algorithm where a `write_env` is incorrectly
...
<a name="poposed-wording-for-bulk"></a>

[Change [exec.bulk]{.sref} paragraphs 3 and 4 as follows:]{.ednote}
...
> ::: add
> 
> 5. For subexpressions `sch`, `sndr`, and `env`, let `Sndr` be `decltype((sndr))`{.cpp},
>    and let ```@_`bulk-transform`_@(sch, sndr, env)```{.cpp} be expression-equivalent to:
>
>    - [5.1] ```sch.@_`bulk-transform`_@(sndr, env)```{.cpp} if that expression is well-formed;
>      otherwise,
>
>    - [5.2] ```sch.@_`bulk-transform`_@(@_`as-bulk-chunked`_@(sndr), env)```{.cpp} if
>      that expression is well-formed; otherwise,
> 
>    - [5.3] ```@_`bulk-transform`_@(sch, sndr, env)```{.cpp} is ill-formed.
> 
> 6. Let `sndr` and `env` be subexpressions and let `Sndr` be `decltype((sndr))`. The
>    expression ```@_`bulk-algo`_@.@_`transform-sender`_@(sndr, env)```{.cpp} is
>    expression-equivalent to:
>
>    - [6.1] ```@_`bulk-transform`_@(get_completion_scheduler<set_value_t>(get_env(sndr)), sndr, env)```{.cpp}
>      if that expression is well-formed, except that `sndr` is evaluated only once;
>      otherwise,
>
>    - [6.2] ```@_`bulk-transform`_@(get_scheduler(env), sndr, env)```{.cpp} if that
>      expression is well-formed, except that `env` is evaluated only once; otherwise,
>
>    - [6.3] ```@_`bulk-algo`_@.@_`transform-sender`_@(sndr, env)```{.cpp} is ill-formed.
>
> :::
...
[Change [exec.task.scheduler]{.sref} as follows:]

[**TODO**]{.blue}


[Change [exec.task]{.sref} as follows:]
